...
> **现场生产库 / MCP 生产数据源不可达时必读。** Agent 凭截图、报错、界面字段等线索做**只读分析**，按问题类型产出代码逻辑分析或排查 SQL 与处置建议；**验证前不改代码**。  
> 有 traceId 且 MCP 可达 → 改走 [workflow.md](../workflow.md#分支-a生产排查不走改码流程) + Skill `his-log-diagnosis`。  
> 规范：`.cursor/rules/zoehis-*.mdc`；表名前缀见 Rule `zoehis-db-tables`、`zoehis-business`。
>
> **排查方式选择原则**：「按钮不显示 / 流程走错 / 计算结果不对 / 异步覆盖」等代码逻辑疑点，优先输出代码逻辑分析；「池表残留 / 状态异常 / 数据对不上」等数据状态疑点，才输出 SQL 脚本包。不要默认给 SQL。
...
> **所有子仓库均位于工作区根目录下**，前后端代码均可直接 Read/Grep，**禁止**用 Glob `**/{repo-name}/**` 搜索（会返回空）。
> 完整映射见 Skill `zoehis-code-map`，以下为排查常用仓库：

| 仓库 | 域 | 路径（workspaceRoot 下） |
|------|-----|----------|
| onelink-web-cis-common | 公共前端组件 | `{workspaceRoot}/onelink-web-cis-common/` |
...
| onelink-micro-charge-fj-common | **收费后端** | `{workspaceRoot}/onelink-micro-charge-fj-common/` |
...
**定位步骤**：
1. 从前端 API 的 `baseUrl` 推断后端服务名（如 `/charge-service/...` → `onelink-micro-charge-fj-common`）
2. 在后端仓库内 Grep 接口路径或方法名定位 Controller → Service → Dao → *Dao.xml
3. 若本地仓库不存在，标「待现场提供后端代码」，**不要**直接跳到 MCP get_code
...
| **代码逻辑疑点** | 代码逻辑分析 | 条件分支遗漏、状态码判断错误、异步覆盖、参数默认值问题 |
| **两者皆有** | 代码分析 + SQL 脚本 | 逻辑有分支 + 需确认数据走向 |
| **参数/配置疑点** | 参数核对清单 | 系统参数、页面开关、字典项 |

**判断原则：**
- 问题描述涉及「按钮不显示」「流程走错」「计算结果不对」「异步覆盖/闪烁/消失」→ **优先代码逻辑分析，默认不写 SQL**
- 问题描述涉及「数据不对」「库里查不到」「状态异常」→ **优先 SQL**
- 用户或现场已明确排除数据/后端问题 → **只做代码逻辑分析**
- 不确定时 → **两者结合**，先代码分析定位关键点，再出针对性 SQL

> **注意**：本流程不默认产出 SQL 脚本包。Agent 应先判断问题类型，非数据类疑点不必输出大段 SELECT 脚本。
...
| 表结构（测试库） | MCP `zoe-his-mcp` → `get_table_schema`（`dev/mcp/zoe-his-mcp/`） |
