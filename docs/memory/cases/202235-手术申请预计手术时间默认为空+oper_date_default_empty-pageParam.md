...
> **文件名**：`202235-手术申请预计手术时间默认为空+oper_date_default_empty-pageParam.md`

> 状态：`verified`  
> 日期：2026-06-06  
> 域：住院 / 处方  
> **IMA 已上传**：`yes`  
> **IMA note_id**：`7479841641224468`  
> **IMA 目录**：需求开发 / 手术  
> **IMA 上传日期**：2026-07-06
...
3. **使用处条件判断**：
   ...
   - **子组件** `operateApplication.vue` → `operationClassSelect`：择期不自动填时需同样判断（见 [闭环 case](2026-06-operationApply-send-null-date-npe.md)）
...
- **后续（2026-06-24）**：子组件 `operateApplication.vue` 择期自动填时间、临床路径 send NPE → 见 [闭环 case](2026-06-operationApply-send-null-date-npe.md)
