...
> **`wecom_record_id` 列**：已上传企业微信文档时填写 record_id；`—` 表示未上传。与 case 元数据 `企业微信已上传：yes` 双重防重复。

| 日期 | 禅道/需求 | 域 | 关键词 | 案例文件 | IMA note_id | wecom_record_id | 已升格 |
|------|-----------|-----|--------|----------|-------------|--------|
...
| 2026-07-14 | 205367 | 医保/结算清单 | 批量上传、批量归档、勾选数据、getSelectArr(false)、upLoadBatch、archiveBatch、settlementRecordManage、status过滤 | [cases/205367-结算清单批量上传只提交勾选数据+settlementRecordManage-upLoadBatch.md](cases/205367-结算清单批量上传只提交勾选数据+settlementRecordManage-upLoadBatch.md) | — | — | — |
...
| `pages/insurance/healthMaintainManage/settlementRecordManage.vue` | [205367 批量上传只提交勾选](cases/205367-结算清单批量上传只提交勾选数据+settlementRecordManage-upLoadBatch.md)（同页面另有 #205366 stas_type 取视图数据，仍在短期记忆） |
