...
### 2026-06-12 — 业务规则速查表 + Index 反向索引

- **触发**：记忆机制改善建议
- **变更**：
  - 新增 `business-rules.md`：从 16 个 case 中抽取 8 类业务规则（参数体系、费用结算、池表库存、手术医嘱、患者读卡、处方药品、仓库定位、发布 Git）
  - `index.md` 新增「反向索引：按表名」（16 张表）和「反向索引：按页面路由」（15 个页面）
- **确认人**：团队
