...
git reset --hard $new
rm -f .git/COMMIT_MSG_FEATURE.txt   # 提交后立即删除临时文件

# 3. 参数 jsonl 单独一轮（同上，换 COMMIT_MSG_PARAM.txt）
```

- **禁止**在仓库根目录创建 `commit-msg*.txt`（已 `.gitignore`；误提交污染配置仓）
- **禁止** `git filter-branch --msg-filter` + batch 脚本（Windows 易把 cmd 启动 banner 写入 message）
- 优先 `git commit -m "$(cat <<'EOF' ... EOF)"`（HEREDOC）；Windows 下才用 `.git/COMMIT_MSG_*.txt`
...
- **参数规范**：[dev/rules/zoehis-sys-param.mdc](../../rules/zoehis-sys-param.mdc) — 系统参数 vs 页面参数区分 + 提交标准
