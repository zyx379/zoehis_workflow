...
**输出与记忆**：遵循 [docs/workflow.md](../../../docs/workflow.md) **[排查类共用约定](../../../docs/workflow.md#排查类共用约定分支-a--b)**——一次精简回复、排查专用记忆召回、纯排查不写 `docs/memory/cases/`。
...
按 [workflow 排查输出上限](../../../docs/workflow.md#输出精简) 一次给齐：
...
**5. 关键代码/SQL**（≤10 行，仅必要片段）

**6. 改造 prompt**（根因明确为代码缺陷时，可复制 prompt 转 Bug 流程）

**7. 案例沉淀**（见文末；默认不输出 Skill 优化长文）
...
- 纯排查结论 **不写** `docs/memory/cases/`（见 workflow 排查沉淀）  
- 确认可复用 trace/日志模式 → 本文件 `cases.md` 追加简短 CASE  
- 改码交付后 → 开发 case 走 workflow Step 12；IMA 同步 folder=问题排查（用户点名或 CASE 新增时）
