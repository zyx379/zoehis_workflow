...
async function findOutpTables() {
    console.log('🔍 查询门诊相关表...\n');
    
    // 查询所有以 OUTP 开头或包含 CHARGE 的表
    const tablesResult = await queryBusinessData({
        sql: "SELECT TABLE_NAME FROM USER_TABLES WHERE TABLE_NAME LIKE '%OUTP%' OR TABLE_NAME LIKE '%CHARGE%' ORDER BY TABLE_NAME",
        description: '查询门诊和收费相关表',
    });
...
findOutpTables().catch(console.error);
